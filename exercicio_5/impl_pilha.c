...
bool is_equal(Pilha pilha1, Pilha pilha2){
	bool deuCerto = false;
	int top1 = pilha1->topo, top2 = pilha2->topo;

	if(!is_empty(pilha1) && !is_empty(pilha2)){

		do
		{
			if(pilha1->itens[top1] == pilha2->itens[top2]){
				deuCerto = true;
				top1 -= 1;
				top2 -= 1;
			}else{
		...
		} while (deuCerto == true && top1 > -1 && top2 > -1);
